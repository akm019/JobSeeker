import React, { useContext, useState,useEffect } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import { AuthContext } from '../AuthContext';
import GetStared from './GetStared';
import icon from '../assets/icon.jpeg';
import { Search, MapPin, Building } from 'lucide-react';
import axios from 'axios'
import image from '/images1.avif'

import { Star } from 'lucide-react';

const testimonials = [
  {
    name: "Sarah Johnson",
    role: "Software Engineer",
    image: "data:image/jpeg;base64,",
    rating: 5,
    text: "Found my dream job through JobSeeker. The platform's AI-powered matching system is exceptional!"
  },
  {
    name: "Mike Chen",
    role: "Data Scientist",
    image: "data:image/jpeg;base64,",
    
    rating: 5,
    text: "The career guidance and mentorship programs helped me transition into tech smoothly."
  },
  {
    name: "Emma Davis",
    role: "UX Designer",
    image: "data:image/jpeg;base64,",
    rating: 4,
    text: "Great community of professionals. The mock interviews really prepared me for the real thing."
  }
];

const Hero = () => {
  const [showModal, setShowModal] = useState(false);
  const navigate = useNavigate();
  const { user } = useContext(AuthContext);
  const [searchQuery, setSearchQuery] = useState('');
  const [jobs, setJobs] = useState([]);
  const [filteredJobs, setFilteredJobs] = useState([]);
  const [showSearchResults, setShowSearchResults] = useState(false);
  const [currentTestimonial, setCurrentTestimonial] = useState(0);


  useEffect(() => {
    const fetchJobs = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/JobPost');
        setJobs(response.data);
      } catch (error) {
        console.error('Error fetching jobs:', error);
      }
    };
    fetchJobs();
  }, []);


   // Filter jobs based on search query
   useEffect(() => {
    if (searchQuery.trim()) {
      const filtered = jobs.filter(job => {
        const searchTerm = searchQuery.toLowerCase();
        return (
          job.positionTitle.toLowerCase().includes(searchTerm) ||
          job.industry.toLowerCase().includes(searchTerm) ||
          job.description.toLowerCase().includes(searchTerm) ||
          job.companyName.toLowerCase().includes(searchTerm) ||
          job.location.toLowerCase().includes(searchTerm)
        );
      });
      setFilteredJobs(filtered);
      setShowSearchResults(true);
    } else {
      setShowSearchResults(false);
    }
  }, [searchQuery, jobs]);

  const handleSearch = (e) => {
    e.preventDefault();
    navigate(`/JobFind?query=${encodeURIComponent(searchQuery)}`);
  };

  const handleJobClick = (jobId) => {
    navigate(`/JobFind?selected=${jobId}`);
  };
  
  const handleGetStarted = () => {
    if (!user) {
      alert("Kindly SignUp first");
    } else if (user.role === 'jobSeeker') {
      navigate('/JobFind');
    } else {
      setShowModal(true);
    }
  };
  // const [currentTestimonial, setCurrentTestimonial] = useState(0);

  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentTestimonial((prev) => (prev + 1) % testimonials.length);
    }, 5000);
    return () => clearInterval(timer);
  }, []);

  return (
    <div className="min-h-screen  bg-[#020B2D]">
 
      <div className="text-center  mb-16">
        <h1 className="text-5xl font-bold pt-10 text-white mb-4">
          Make Your Dream true
          <span className="block text-[#4361EE] mt-2">With JobSeeker</span>
        </h1>
        <p className="text-gray-300 max-w-2xl mx-auto mb-6">
          JobSeeker presents a casual, joyful character with care-free excellence, providing all the
          features you need to launch your career.
        </p>
        <button 
          onClick={handleGetStarted}
          className="px-8 py-3 bg-[#4361EE] text-white rounded-lg text-lg font-semibold hover:opacity-90 transition-opacity"
        >
          Get Started
        </button>
      </div>

        {/* Search Section */}
        <div className="max-w-4xl mx-auto mb-16">
        <div className="max-w-4xl mx-auto mb-16 relative">
          <form onSubmit={handleSearch} className="bg-white rounded-lg p-4 shadow-lg">
            <div className="flex gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-2 border-r pr-4">
                  <Search className="text-gray-400" />
                  <input
                    type="text"
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    placeholder="Search for jobs, skills, or companies"
                    className="w-full outline-none text-gray-800"
                  />
                </div>
              </div>
              <div className="flex gap-4 items-center">
                <select className="outline-none text-gray-600 border-r pr-4">
                  <option>All Locations</option>
                </select>
                <select className="outline-none text-gray-600">
                  <option>All Departments</option>
                </select>
                <button type="submit" className="px-6 py-2 bg-[#4361EE] text-white rounded-lg">
                  Search
                </button>
              </div>
            </div>
          </form>
          {showSearchResults && filteredJobs.length > 0 && (
            <div className="absolute w-full bg-white mt-2 rounded-lg shadow-lg z-30 max-h-96 overflow-y-auto">
              {filteredJobs.map((job) => (
                <div
                  key={job._id}
                  onClick={() => handleJobClick(job._id)}
                  className="p-4 hover:bg-gray-50 cursor-pointer border-b last:border-b-0"
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="font-semibold text-gray-800">{job.positionTitle}</h3>
                      <p className="text-gray-600">{job.companyName}</p>
                      <div className="flex items-center gap-2 mt-1 text-gray-500 text-sm">
                        <MapPin size={16} />
                        <span>{job.location}</span>
                        <span className="mx-2">•</span>
                        <span>{job.salary}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ))}
            </div>
          )}
          
        </div>
        

        <div className="flex flex-col lg:flex-row gap-8">
          {/* Profile Section */}
          <div className="lg:w-1/4 bg-[#051140] p-6 rounded-xl">
            <div className="flex flex-col items-center">
              {user ? (
                <img 
                src={user && user.picture ? user.picture : icon} 
                alt="Profile" 
                className="w-24 h-24 rounded-full border-4 border-[#4361EE]" 
              />              
              ) : (
                <img src={icon} alt="Default" className="w-24 h-24 rounded-full border-4 border-[#4361EE]" />
              )}
              
              <h3 className="text-2xl font-bold mt-4 mb-6 text-white">My Profile</h3>
              
              <div className="w-full space-y-4 text-gray-300">
                <p><span className="text-white">Name:</span> {user?.name || "John Doe"}</p>
                <p><span className="text-white">Email:</span> {user?.email || "john@example.com"}</p>
                <p><span className="text-white">Skills:</span> {user?.skills.join(",") || "React, Node.js"}</p>
                <p><span className="text-white">Resume Link:</span> {user?.resume}</p>
              
              </div>

              <Link to="/ProfileEdit" className="mt-6 w-full">
                <button className="w-full px-6 py-2 bg-[#4361EE] text-white rounded-lg hover:opacity-90">
                  Edit Profile
                </button>
              </Link>
            </div>
          </div>

          {/* Main Content */}
          <div className="flex-1 grid gap-6">
            <div className="grid md:grid-cols-2 gap-6">
              {['Twitter', 'Reddit', 'Google'].map((company) => (
                <div key={company} className="bg-[#051140] p-6 rounded-xl">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="text-white font-bold">{company}</h3>
                      <p className="text-gray-300">Software Engineer</p>
                    </div>
                    <button className="px-4 py-1 bg-[#4361EE] text-white rounded-lg text-sm">
                      Apply
                    </button>
                  </div>
                  <div className="mt-4 text-gray-300">
                    $80k - $120k
                  </div>
                </div>
              ))}
            </div>

            {/* Quick Access Cards */}
            <div className="grid md:grid-cols-3 gap-6">
              <div className="bg-[#051140] p-6 rounded-xl">
                <h3 className="text-white font-bold mb-3">Are You a Fresher?</h3>
                <p className="text-gray-300 mb-4">Get industry ready with our courses 📚</p>
                <Link to='/Courses'>
                  <button className="px-6 py-2 bg-[#4361EE] text-white rounded-lg hover:opacity-90">
                    Explore
                  </button>
                </Link>
              </div>

              <div className="bg-[#051140] p-6 rounded-xl">
                <h3 className="text-white font-bold mb-3">Looking for Guidance?</h3>
                <p className="text-gray-300 mb-4">Chat with industry experts</p>
               <Link to ="/ChatRooms">
               <button className="px-6 py-2 bg-[#4361EE] text-white rounded-lg hover:opacity-90">
                  Connect
                </button></Link>
              </div>

              <div className="bg-[#051140] p-6 rounded-xl">
                <h3 className="text-white font-bold mb-3">Test Your Prep</h3>
                <p className="text-gray-300 mb-4">Take mock interviews</p>
                <button className="px-6 py-2 bg-[#4361EE] text-white rounded-lg hover:opacity-90">
                  Start Now
                </button>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="bg-white py-16">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex flex-col md:flex-row items-center gap-12">
            <div className="flex-1">
              <h2 className="text-3xl font-bold text-[#020B2D] mb-8">What Our Users Say</h2>
              
              <div className="bg-[#F8F9FF] p-8 rounded-2xl shadow-lg">
                <div className="flex items-center gap-4 mb-4">
                  <img 
                    src={testimonials[currentTestimonial].image} 
                    alt={testimonials[currentTestimonial].name}
                    className="w-16 h-16 rounded-full"
                  />
                  <div>
                    <h3 className="font-bold text-[#020B2D]">{testimonials[currentTestimonial].name}</h3>
                    <p className="text-gray-600">{testimonials[currentTestimonial].role}</p>
                  </div>
                </div>
                
                <div className="flex gap-1 mb-4">
                  {[...Array(testimonials[currentTestimonial].rating)].map((_, i) => (
                    <Star key={i} className="fill-[#4361EE] text-[#4361EE]" size={20} />
                  ))}
                </div>
                
                <p className="text-gray-700 text-lg italic">"{testimonials[currentTestimonial].text}"</p>
                
                <div className="flex gap-2 mt-6">
                  {testimonials.map((_, index) => (
                    <button
                      key={index}
                      className={`w-3 h-3 rounded-full transition-colors ${
                        index === currentTestimonial ? 'bg-[#4361EE]' : 'bg-gray-300'
                      }`}
                      onClick={() => setCurrentTestimonial(index)}
                    />
                  ))}
                </div>
              </div>
            </div>
            
            <div className="flex-1">
              <img 
                src={image}
                alt="Success Stories"
                className=""
              />
            </div>
          </div>
        </div>
      </div>
      {showModal && <GetStared onClose={() => setShowModal(false)} />}
    </div>
  );
};

export default Hero;