...
function ChatRooms() {
  ...
  const checkAllEnrollments = async () => {
    try {
      ...
      const statusPromises = rooms.map(room => 
        fetch(`https://jobseeker-1-1buy.onrender.com/api/rooms/${room._id}/enrollment-status`, {
          headers: {
  ...
  const createRoom = async (e) => {
    ...
    try {
      const response = await fetch('https://jobseeker-1-1buy.onrender.com/api/rooms', {
        method: 'POST',
        headers: {
  ...
  const fetchRooms = async () => {
    try {
      const res = await fetch('https://jobseeker-1-1buy.onrender.com/api/rooms', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const handleEnroll = async () => {
    ...
    try {
      const res = await fetch(`https://jobseeker-1-1buy.onrender.com/api/rooms/${selectedRoom._id}/enroll`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
