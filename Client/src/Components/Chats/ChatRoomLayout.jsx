...
function ChatRoomLayout() {
  ...
  const fetchRoomDetails = async () => {
    try {
      const res = await fetch(`https://jobseeker-1-1buy.onrender.com/api/rooms/${roomId}`, {
        headers: {
  ...
  const removeParticipant = async (participantId) => {
    try {
      const res = await fetch(
        `https://jobseeker-1-1buy.onrender.com/api/rooms/${roomId}/participants/${participantId}`,
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const fetchParticipants = async () => {
    try {
      const res = await fetch(`https://jobseeker-1-1buy.onrender.com/api/rooms/${roomId}/participants`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const checkEnrollmentStatus = async () => {
    try {
      const res = await fetch(`https://jobseeker-1-1buy.onrender.com/api/rooms/${roomId}/enrollment-status`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const handleEnroll = async () => {
    try {
      const res = await fetch(`https://jobseeker-1-1buy.onrender.com/api/rooms/${roomId}/enroll`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
