...
const PersonalChat = ({ participant, onClose, currentUser }) => {
  ...
  const fetchPersonalMessages = async () => {
    try {
      setIsLoading(true);
      const res = await fetch(`https://jobseeker-1-1buy.onrender.com/api/personal-messages/${participant._id}`, {
        headers: {
  ...
  const deleteMessage = async (messageId) => {
    try {
      const res = await fetch(`https://jobseeker-1-1buy.onrender.com/api/personal-messages/${messageId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
  ...
  const sendMessage = async (e) => {
    ...
    try {
      ...
      const res = await fetch('https://jobseeker-1-1buy.onrender.com/api/personal-messages', {
        method: 'POST',
        headers: {
